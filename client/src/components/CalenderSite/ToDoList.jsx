...
const ToDoList = ({
  ...
  return (
    <div
      ref={listRef}
      className={`relative group m-3 mb-2 bg-white outline-[0.5px] outline-[#484848] rounded-t-[25px] rounded-b-[10px] shadow-sm shadow-gray-400 ${isDraggingOver ? "bg-blue-50" : ""}`}
    ...
    >
      ...
      <div className="mt-2 overflow-y-auto custom-scrollbar" style={{ maxHeight: `calc(${heightToUse} - 75px)`, paddingRight: "20px" }}>
        {taskInputs.map((task) => (
          <div key={task.id} className={`flex items-center space-x-3 mb-2 w-full ${task.color}`} draggable onDragStart={(e) => handleDragStart(e, task.id, task)}>
            <button
            ...
            <input
              ...
              className={`p-2 border border-gray-300 rounded w-full transition-all duration-300 ease-in-out transform ${task.animate ? "animate-slide-down" : ""} ${task.completed ? "line-through text-gray-500 opacity-60" : ""} cursor-grab`}
